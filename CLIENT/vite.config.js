...
// ← ADD THIS LINE AT THE VERY TOP
import path from "path"; // ← Node.js built-in (no install needed)
...
export default defineConfig({
  ...
  // ← ADD THIS WHOLE BLOCK (just 5 lines)
  resolve: {
    alias: {
  ...
  // ← Your existing proxy stays exactly the same (login/auth still works)
  server: {
    proxy: {
      "/api": {
        target: "http://localhost:5000",
