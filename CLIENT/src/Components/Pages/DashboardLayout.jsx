...
import { useAuth } from "../../context/useAuth.js";
import { LogOut, LayoutDashboard, HeartHandshake } from "lucide-react"; // Heart icon for blood donation theme
...
const DashboardLayout = ({ children }) => {
  ...
  return (
    <div className="font-inter min-h-screen bg-gradient-to-br from-red-50 via-white to-red-100 dark:from-gray-900 dark:via-gray-800 dark:to-red-900 transition-colors duration-500">
      {/* Floating Navigation Bar */}
      <nav
        className={`
          fixed top-4 left-1/2 transform -translate-x-1/2 z-50
          w-fit py-3 px-6
          bg-red-700/90 backdrop-blur-lg text-white
      ...
      >
        <div className="flex justify-between items-center space-x-12">
          {/* Dashboard Title */}
          <div className="flex items-center space-x-3">
            <LayoutDashboard className="w-6 h-6 text-yellow-300" />
            <h1 className="text-xl font-extrabold tracking-tight whitespace-nowrap">
              {capitalizedRole} Dashboard
            </h1>
          </div>
          ...
          <div className="flex items-center space-x-6">
            ...
            <button
              ...
              className={`
                ...
                text-white font-medium py-2 px-3 rounded-xl
                transition-all duration-300 shadow-md hover:shadow-lg hover:scale-[1.05]
                whitespace-nowrap
              `}
              title="Logout"
            >
