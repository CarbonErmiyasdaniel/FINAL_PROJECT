          ...
          <div className="pt-4 border-t border-red-700">
            ...
            >
              <div className="relative group">
                ...
                />
                {/* Upload Feature Placeholder (Front Part) */}
                <button
                  className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-full flex items-center justify-center text-white text-xs font-semibold"
                  aria-label="Upload Profile Image"
                >
...
const HospitalStaff = ({ pageKey }) => {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (!desktopMode) {
        ...
        // Do not read isSidebarOpen here to avoid stale closure / lint warning;
        // hover handlers will expand the sidebar when needed.
      }
    };
    handleResize();
    window.addEventListener("resize", handleResize);
  ...
  const handleMouseLeave = () => {
    // Only collapse if the sidebar was expanded by hover (not forced open on mobile)
    if (isDesktop) {
      setIsSidebarOpen(false);
    }
  };
  ...
  const handleNavigate = (path) => {
    // Acknowledging Saved Information: I note that you have no issue proceeding to the next step (front end part) from your saved instructions.
    if (!isDesktop) {
      setIsSidebarOpen(false);
    }
    setCurrentPage(path);
  ...
  // --- Omitted DashboardContent and RenderPage for brevity, as they were not requested for UI change ---
  const DashboardContent = () => (
    <>
      <div className="flex justify-between items-start mb-6">
        <div>
    ...
    <div className="flex h-screen bg-gray-100 dark:bg-gray-900 font-inter">
      ...
      >
        ...
        <div className="space-y-4">
          ...
          {/* Patient Care Section - Clean Separation */}
          <section className="pt-4 border-t border-red-700">
            <h2
              className={`text-sm font-semibold text-red-200 uppercase tracking-wider mb-2 transition-opacity duration-300 ${
                isSidebarOpen ? "opacity-100 block px-4" : "opacity-0 hidden"
          ...
          {/* Donor Operations Section - Clear Separation */}
          <section className="pt-4 border-t border-red-700">
            <h2
              className={`text-sm font-semibold text-red-200 uppercase tracking-wider mb-2 transition-opacity duration-300 ${
                isSidebarOpen ? "opacity-100 block px-4" : "opacity-0 hidden"
            ...
            <div className="flex flex-col gap-1">
              ...
              <SidebarButton
                ...
                isActive={currentPage === "/hospital_staff/Donor_List"}
              />
              {/* Added Blood Stock Status for relevance */}
              <SidebarButton
                onClick={() => handleNavigate("/hospital_staff/blood_stock")}
                icon={<Heart className="h-5 w-5" />}
                label="Current Blood Stock"
          ...
          {/* Configuration Section - Bottom-aligned (still within the scrollable area) */}
          <section className="pt-4 border-t border-red-700">
            <h2
              className={`text-sm font-semibold text-red-200 uppercase tracking-wider mb-2 transition-opacity duration-300 ${
                isSidebarOpen ? "opacity-100 block px-4" : "opacity-0 hidden"
      ...
      {/* Main content area needs margin/padding to prevent obstruction by the fixed sidebar */}
      <div
        className={`flex-1 flex flex-col overflow-y-auto transition-all duration-300 ${
          isDesktop ? (isSidebarOpen ? "sm:ml-72" : "sm:ml-20") : "sm:ml-0"
        }`}
      >
        {/* Mobile Header - Improved styling for cohesiveness */}
        <header className="flex items-center justify-between p-4 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 h-16 shadow-lg sm:hidden">
          <div className="flex items-center">
            ...
            <span className="text-xl font-bold text-red-700">BLOODNET</span>
          </div>
        </header>
        <main className="flex-1">{RenderPage()}</main>
      </div>
