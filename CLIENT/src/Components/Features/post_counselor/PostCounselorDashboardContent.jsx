...
const PostCounselorDashboardContent = () => {
  ...
  const fetchPending = async () => {
    try {
      ...
      const response = await axios.get("/api/post-counselor/pending", {
        // Use Authorization header for robust authentication
        headers: {
    ...
    <div className="space-y-10">
      ...
      <div className="space-y-8">
        {pending.map((item) => {
          ...
          // FIX APPLIED HERE: Check if testedAt is valid before calling format
          const testedDate = item.testedAt
            ? format(new Date(item.testedAt), "dd MMM yyyy, HH:mm")
            : "Date N/A";
