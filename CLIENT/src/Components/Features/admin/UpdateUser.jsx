...
const UpdateUser = ({ user, onClose, onUserUpdated }) => {
  // Initialize state with phone added
  const [formData, setFormData] = useState({
    name: user.name || "",
    email: user.email || "",
    role: user.role || "donor",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Validate phone (E.164 minimal front-end check)
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div className="pt-4 border-t border-gray-200">
              <button
                type="submit"
                disabled={isLoading}
                className={`w-full py-3 px-4 rounded-lg text-white font-semibold flex items-center justify-center gap-3
                  ${
                    isLoading
                      ? "bg-gray-400"
