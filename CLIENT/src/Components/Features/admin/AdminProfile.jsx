...
const AdminProfile = () => {
  ...
  const handleUploadPhoto = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await axios.patch("/api/admins/photo", formData, {
        // Updated endpoint
        headers: {
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.patch(
        "/api/admins/change-password", // Updated endpoint
...
//     try {
//       const token = localStorage.getItem("token");
//       const res = await axios.patch("/api/admin/photo", formData, {
//         headers: {
//           Authorization: `Bearer ${token}`,
//           "Content-Type": "multipart/form-data",
//         },
...
//   // Check if loading or user is null before rendering
//   if (loading) {
//     return (
//       <div className="flex items-center justify-center min-h-screen">
//         <div className="text-2xl text-gray-600">Loading profile...</div>
