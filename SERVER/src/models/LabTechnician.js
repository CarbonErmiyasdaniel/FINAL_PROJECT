...
// Ensure one user has only one lab technician profile
LabTechnicianSchema.index({ user: 1 }, { unique: true });

// Static method to get the average experience of all lab technicians
LabTechnicianSchema.statics.getAverageExperience = async function() {
  ...
  try {
    await this.model('LabTechnicianStats').findOneAndUpdate(
      {},
      {
...
LabTechnicianSchema.post('save', function() {
  this.constructor.getAverageExperience();
});

// Call getAverageExperience before remove
LabTechnicianSchema.pre('remove', function() {
  this.constructor.getAverageExperience();
});
